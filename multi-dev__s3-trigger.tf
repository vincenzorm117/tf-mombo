...
data "aws_iam_policy_document" "multi-dev-deploy" {
  ...
  statement {
    ...
    effect = "Allow"
    resources = [
      for d in data.aws_route53_zone.main : "arn:aws:route53:::hostedzone/${d.zone_id}"
    ]
  }
}
...
resource "aws_lambda_function" "multi-dev-deploy" {
  function_name = "multi-dev-deploy"
    ...
    variables = merge(
      {
        ...
        for s in var.static_sites : data.aws_route53_zone.main[regex("[^.]+.[^.]+$", s.hostname)].zone_id => "hz-${aws_s3_bucket.multi[s.hostname].bucket}"
        }, {
